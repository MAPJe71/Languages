...
@=MATLAB - MATrix LABoratory
  ...
  all_matlab_keywords:
      ...
      matlab_support_toolbox_virtual_reality            : (?<!\.)\b(vrworld|vrwhos|vrwho|vrview|vrspacemouse|vrsetpref|vrrotvec2mat|vrrotvec|vrrotmat2vec|vrplay|vrori2dir|vrnode|vrlib|vrjoystick|vrinstall|vrgetpref|vrfigure|vrdrawnow|vrdir2ori|vrclose|vrclear)\b
      ...
      matlab_variable_function                          : (?<!\.)\b(nargin|nargout|varargin|varargout)\b
  matlab_oop:
    - match: \b(classdef)\b
    ...
    - match: \b(properties|events|enumeration)\b
      scope: keyword.other.oop.matlab
  special_characters    : '((\%([\+\-0]?\d{0,3}(\.\d{1,3})?)(c|d|e|E|f|g|G|s|((b|t)?(o|u|x|X))))|\%\%|\\(b|f|n|r|t|\\))'
  variable              : '\b[a-zA-Z]\w*\b'
  variable_invalid      : '\b(_\w|\d+[_a-df-zA-DF-Z])\w*\b'  comment: No variables or function names can start with a number or an underscore.
       ...
       (?s:%\{(?:[^%]|%[^}])*?%})
        (?s:%\{.*?%\})
								(?s:                                            # Multi Line Comment
									...
									|	%(?![{}])                               # ...not being a start- or end-of-comment indicator
									)*
