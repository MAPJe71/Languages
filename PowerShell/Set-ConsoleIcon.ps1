...
##
## Description: Set icon of current console window to specified icon
...
## Example    : PS:1 > Set-ConsoleIcon "C:\Icons\special_powershell_icon.ico"
##
##############################################################################
...
function Set-ConsoleIcon
{
    ...
    if ([System.IO.File]::Exists($iconFile) -eq $TRUE)
    {
        ...
        if ($icon -ne $null)
        {
            ...
            $null = SendMessage $consoleHandle $WM_SETICON $ICON_SMALL $icon.Handle
        }
    }
    else
    {
