...
assert(_VERSION == "Lua 5.1", "This module requires Lua 5.1")
...
a=1
while a~=5 do -- Note: Lua uses ~= to indicate not equal
    a=a+1
    io.write(a.." ")
end
...
for i =1, #codeSamples do
    ...
    ok, err = pcall(loadstring(codeSamples[i]) )
    if not ok then
        print("failed to load sample code")
        print(err)
    end
